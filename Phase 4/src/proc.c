...
enum {HUNGRY, THINKING, EATING} phstate[5];
// struct spinlock phmutex;
...
void cond_wait(int i) {
  // cprintf("cond_wait phil : %d\n", i);
	self[i].count++;
	if (next_count > 0)
    sem_release(next);
	else
    sem_release(mutex);
	sem_acquire(self[i].sem_no);
  phstate[i] = HUNGRY;
	self[i].count--;
}
	
void cond_signal(int i) {
	if (self[i].count > 0) {
		next_count++;
		sem_release(self[i].sem_no);
		sem_acquire(next);
...
void test(int i)
{
	if((phstate[(i + 1) % 5] != EATING) &&
		(phstate[(i + 4) % 5] != EATING) &&
		(phstate[i] == HUNGRY)) {
		phstate[i] = EATING;
    // acquire(&ptable.lock);
    cprintf("phil %d is EATING!\n", i);
    // release(&ptable.lock);
		cond_signal(i);
	}
}